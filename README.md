...
- **Seeds rows it doesn't have a rule for, by asking the database.** When a table's constraints defeat a templated insert, a probe-and-repair synthesizer tries the insert, reads the real error, and fixes it — filling `NOT NULL` columns, seeding single and **composite** foreign-key parents, varying values to clear `UNIQUE` conflicts, and neutralizing a `CHECK` that delegates to a function (then restoring it) — so even an opaque-function-gated table with a composite FK and a function-backed CHECK still gets a valid row to test against, with no per-schema hand-coding.
