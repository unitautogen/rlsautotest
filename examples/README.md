...
- **rbac_tenant.sql** — multi-tenant RBAC patterns distilled from a real Supabase RBAC extension (opaque, claims-driven `is_member` / `has_role` / `has_permission` helpers with a JWT-expiry guard). Exercises, all on one schema: a **login-only INSERT** (`auth.uid() IS NOT NULL`) beside helper-gated SELECT/UPDATE/DELETE plus an **ownership-on-insert trigger** (`orgs`); a **multi-column UNIQUE** + a `roles text[]` column + a mixed `has_role(...) OR user_id = auth.uid()` DELETE (`mships`); a `roles text[]` with `DEFAULT '{}'` but `CHECK (cardinality(roles) > 0)` (`invs`); and a **composite FK** + a `FOR ALL` policy with **no UPDATE grant** + a `service_role USING (true)` policy (`mperms`). Green — the regression guard that each of these real-world shapes is classified per command, seeded (identities into `auth.users`, `exp` on the JWT, non-empty arrays for the cardinality CHECK), and tested correctly (grant-aware mocking; a service_role-only policy never spawns a bogus authenticated branch).
